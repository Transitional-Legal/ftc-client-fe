...
const Dashboard = () => {
	...
	const { data: settings, error: fetchSettingsError } = useSWR(`/settings/${user.id}`);

	const { data: referralRate, error: fetchReferralRate } = useSWR("/referral/rate");
	const isFetchingSettings = !settings && !fetchSettingsError && !fetchReferralRate;

	const { data: accountInfo, error: fetchAccountInfoError } = useSWR(`/accountInfoes/user/${user.id}`);

	const isFetchingAccountInfo = !accountInfo && !fetchAccountInfoError;

	const fullName = [userDetails?.firstName, userDetails?.middleName, userDetails?.lastName].filter((n) => n).join(" ");

	const mobile = userDetails?.mobileNumber ? parsePhoneNumberFromString(userDetails?.mobileNumber, "AU").format("INTERNATIONAL") : "";
	...
	const payrollColumns = [
		...
		["Deposit Reference (Wage transfer description or staff number as it appears on your bank statement)", depositHints?.bankStatement]
	];

	const accountInfoColumns = [
		["BTC Threshold (Keep Bitcoin Pay on GPIB Portal until they reach this value.)", format$(accountInfo?.btcThreshold, { code: "AUD" })]
	];
	...
	const onEditPayrollClick = (_e) => history.push(`${location.pathname}/payroll/edit`);

	const onUpdatePasswordClick = (_e) => {
		history.push("/auth/resetpassword");
	};
	const onEditReferralClick = (_e) => history.push(`${location.pathname}/referral/send`);

	const onEditMobileClick = (_e) => history.push(`${location.pathname}/mobile/send`);

	const onEditAccountInfoClick = (_e) => history.push(`${location.pathname}/accountInfo/edit`);

	const referralColumns = [
		["Referral Code", `${user.id}`],
		["Referral Link", `${process.env.REACT_APP_URL}/register?referralCode=${user.id}`],
		...
		<Layout activeTab="profile">
			<div className="container py-5">
				<Card>
					...
					<p style={{ fontSize: "95%" }}>
						<i>
							Please <Link to="/contactsupport">contact support</Link> if any information is incorrect or needs to be updated.
						</i>
					</p>
				</Card>
				<Card>
				...
				<Card>
					...
					<Button variant="primary" className="mt-3" onClick={onUpdatePasswordClick}>
						Reset Password
					</Button>
				</Card>
			</div>
